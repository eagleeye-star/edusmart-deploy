...
export async function migrateLocalDataIntoSync({ engine, oldLocalData }) {
  const results = {};
  const tables = ["students", "attendance", "grades", "fees", "staff", "fee_types", "payroll", "books", "borrows", "mock_exams", "expenses", "exam_schedule", "nursery_logs", "milestones"];
...
export async function joinExistingSchoolAndPullData({ auth, storage, remote, deviceEmail, devicePassword }) {
  ...
  try {
    const pulled = {};
    for (const table of ["staff", "students", "attendance", "grades", "fees", "fee_types", "payroll", "books", "borrows", "mock_exams", "expenses", "exam_schedule", "nursery_logs", "milestones"]) {
